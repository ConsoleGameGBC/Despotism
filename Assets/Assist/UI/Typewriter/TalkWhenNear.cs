...
public class TalkWhenNear : MonoBehaviour {
	...
	public void OnTriggerStay() {
		if(orientSpeechBubbleToFaceCamera) {
			if(speechBubble != null) {
				speechBubble.LookAt(Camera.mainCamera.transform.position);
			}
		}
	}
	...
	void HideSpeechBubble() {
		if(speechBubble != null) {
			foreach(var t in speechBubble.transform) {
				(t as Transform).gameObject.renderer.enabled = false;
			}
		}
	}
	...
	void ShowSpeechBubble() {
		if(speechBubble != null) {
			foreach(var t in speechBubble.transform) {
				(t as Transform).gameObject.renderer.enabled = true;
			}
		}
	}
